...
public abstract class CreeperEntityMixin {
    ...
    private Collection<StatusEffectInstance> makeEffectsCloud(Collection<StatusEffectInstance> oldEffects) {
        if ((Mystical.isClientWorld() && Mystical.getSpellHandler().isConsequenceActive(RandomCreeperEffectCloudsConsequence.class))) { // TODO: Config (chance, allowed effects))
    ...
    int modifyExplosionRadius(CreeperEntity instance){
        if ((Mystical.isClientWorld() && Mystical.getSpellHandler().isConsequenceActive(BigCreeperExplosionConsequence.class)) &&
